...
## How radio selection and initialization works in the STM32Cube LoRaWAN stack
#### Radio shield selection
- Radio shield selection (e.g. `SX1272MB2DAS`, `SX1276MB1LAS`, `SX1276MB1MAS`, etc.) is handled through **preprocessor macros** defined in the project configuration.
- These macros are added as **compiler symbols** and can be viewed or modified in:  
  **Project Properties → C/C++ Build → Settings → MCU GCC Compiler → Preprocessor**
#### Radio board interface mapping
- Based on the radio board macro that is defined, the file  
  `Applications/LoRaWAN/Target/radio_board_if.h`  
  selects and includes the appropriate **radio board–specific header file**.
- This header file abstracts the radio board and determines which **SX127x transceiver** implementation is used.
#### why radio.c (not verified)
...
#### SX127x transceiver driver selection
- The selected radio board header internally maps to the correct **SX127x transceiver driver** (for example, `sx1276`).
- The actual transceiver driver implementation resides in:  
  `Middlewares/SubGHz_Phy/sx1276/sx1276.c`
#### Radio initialization in the LoRaWAN stack
- During LoRaWAN stack initialization, the radio driver is initialized using:
```c
Radio.Init(&MacCtx.RadioEvents);
```
- This call is made inside the LoRaMacInitialization() function, located in:
Middlewares/LoRaWAN/Mac/LoRaMac.c
- At this stage, the radio driver is fully initialized and registered with the LoRaMAC layer, enabling communication between the LoRaWAN stack and the selected SX127x radio hardware.  
